...
export async function loginAsBarangay(page: Page): Promise<void> {
  const testEmail = process.env.BARANGAY_EMAIL;   // construct the dynamic email address
  const password = process.env.BARANGAY_PASSWORD;   // use the predefined password
