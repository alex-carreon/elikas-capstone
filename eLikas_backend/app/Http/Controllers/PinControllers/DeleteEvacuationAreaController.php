...
class DeleteEvacuationAreaController extends Controller
{
    ...
    public function restoreEvacuationArea(Request $request, $id)
    {
        try {
            ...
            if (!$pin) {
                return response()->json(['error' => 'Evacuation area not found'], 404);
            }

            if (!$pin->social_element) {
                return response()->json(['error' => 'Evacuation area has no linked social element'], 422);
            }
            ...
            // If expiry has already passed, clear it so the pin shows as active on the map
            if ($pin->expiry !== null && $pin->expiry->lte(now('UTC'))) {
                $pin->expiry = null;
                $pin->save();
            }
            ...
            return response()->json([
                ...
                'expiry'         => $pin->expiry
                    ...
                    : null,
                'deactivated_at' => null,
                'restored_by'    => ['name' => $this->displayName($user)],
            ], 200);
